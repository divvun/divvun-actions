...
async function config() {
    ...
    if (typeof unknown === "object") {
        if (!(Array.isArray(unknown.targets) && unknown.targets.every(t => typeof t === "string"))) {
...
export async function runDivvunRuntimePublish() {
    ...
    for (const target of cfg.targets) {
        ...
        const outPath = `${archivePath.path}/divvun-runtime-${target}-${builder.env.tag!}.${ext}`
        const inputPath = `${tempDir.path}/divvun-runtime${target.includes("windows") ? ".exe" : ""}`
...
class GitHub {
    ...
    async createRelease(tag: string, artifacts: string[], draft = false, prerelease = false) {
        const args = [
            "release",
            "create",
            tag,
        ...
        logger.info(`Creating GitHub release: gh ${args.map(a => `"${a}"`).join(" ")}`)
