...
async function loadCargoToml(): Promise<any> {
  const cargoString = await Deno.readTextFile("./divvunspell/Cargo.toml")
...
async function createTarball(
  ...
  // Copy lib directory contents to temp directory
...
export default async function divvunspellDeploy({
  libPath,
  version,
  channel,
  pahkatRepo,
...
export async function runDivvunspellDeploy() {
  ...
  try {
    ...
    throw new Error("No lib directory found in artifacts - Android build may have failed")
  }
