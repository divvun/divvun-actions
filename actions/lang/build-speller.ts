...
export default async function langSpellerBuild(
  ...
  // Upload all files generated by autogen.sh/configure that are not tracked by
  // git (e.g. configure, aclocal.m4, Makefile.in files, build-aux/, config.h.in).
  // Using --others without --exclude-standard ensures gitignored generated files
  // are included. build/ is already handled above. autom4te.cache/ is autoconf's
  // local cache and is not needed on the test agent.
  const { stdout: gitLsOut } = await new Deno.Command("git", {
  ...
  const untrackedFiles = new TextDecoder()
    ...
    .filter((f) =>
      f.length > 0 &&
      !f.startsWith("build/") &&
      !f.startsWith("autom4te.cache/")
    )

  if (untrackedFiles.length > 0) {
    await builder.uploadArtifacts(untrackedFiles.join(";"))
  }
